...
class AdaptiveLRManager:
    ...
    def on_epoch_end(self, val_loss):
        ...
        if no_improvement or increasing:
            ...
            print(f"  ⚠️ Loss {'上升' if increasing else '平原'} ({self.plateau_counter}/{self.config.plateau_patience})")
        else:
        ...
        if self.current_multiplier < 0.01 or self.lr_reduce_count >= self.max_lr_reduces:
            print(f"⏹️ 学习率过低 (乘数: {self.current_multiplier:.3f})，触发早停")
            should_stop = True
        return should_stop
...
def pretrain(model, train_gen, val_gen, vocab_size, config, save_dir="output/pretrain"):
    print("\n🚀 开始预训练")
    ...
    with open(sft_path, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            if line.strip():
                try:
                    item = json.loads(line)
                    total_lens.append(len(item.get('prompt', '')) + len(item.get('response', '')))
                except:
                    continue
    if not total_lens:
        return None
    p95 = int(np.percentile(total_lens, 95))
    seq_len = min(max(p95 + 10, 128), 512)
    seq_len = (seq_len // 8) * 8
    return seq_len
...
def main():
    ...
    if estimated_seq_len and estimated_seq_len > config.seq_len:
        print(f"📊 SFT 数据需要 seq_len={estimated_seq_len}，预训练将统一使用此长度")
        config.seq_len = estimated_seq_len
    ...
    print(f"📊 预训练语料总长度: {corpus_chars:,} 字符 ({corpus_chars/1e6:.1f}MB)")

    if corpus_chars < 1000:
        print("⚠️ 预训练数据过少，补充测试数据")
