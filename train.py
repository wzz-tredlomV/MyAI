...
def train_epoch(
    encoder, generator, discriminator, dataloader, optimizers, device, epoch
):
    """训练一个epoch"""
    encoder.train()
    generator.train()
    ...
    for batch in pbar:
        ...
        recon_loss = nn.L1Loss()(
            generated["presence_map"], (hw > 0).float().unsqueeze(1)
        )

        # 风格一致性
        style_loss = StyleConsistencyLoss()(global_style, encoder(hw)["global"])
